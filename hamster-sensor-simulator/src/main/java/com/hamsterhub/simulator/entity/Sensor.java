...
public final class Sensor {
    ...
    public SensorStatus status() { return status; }
    public void status(SensorStatus s) { this.status = s; }
    ...
    public Mono<Void> relay(WebClient client, HamsterEvent event) {
        if (status != SensorStatus.WORKING) return Mono.empty();
        if (!tryAcquireEmission()) return Mono.empty();
    ...
    /** Попытка «сломаться» с вероятностью perSec */
    public Mono<Void> maybeFail(WebClient client, double failPerSec, Supplier<Double> rnd) {
        if (status != SensorStatus.WORKING) return Mono.empty();
        if (rnd.get() >= failPerSec) return Mono.empty();

        status = SensorStatus.BROKEN;
        return client.post()
                .uri("/events")
                .bodyValue(new SensorFailure(id, 5001))
                .retrieve()
                .bodyToMono(Void.class)
                .onErrorResume(e -> { System.err.println("Failure POST failed: " + e); return Mono.empty(); });
    }
}
